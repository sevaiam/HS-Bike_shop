...
class BikeShopTest(DjangoTest):
    ...
    def _check_button(self, page, button_value):
        button = f'<button type="submit">{button_value}</button>'.lower()
        inp = f'<input type="submit" value="{button_value}">'.lower()
        return button in page.lower() or inp in page.lower()
                ...
                if enough_parts:
                    ...
                    for ind, label in enumerate(labels):
                        ...
                        try:
                            ...
                            return CheckResult.wrong('Make sure the form is correct. Check if <p> tags are used to display form elements and the form has three fields.')
                    ...
                    sentence = "<b>Unfortunately, this bike is currently not available.</b>"
                    if sentence in p_tags:
                        return CheckResult.correct()
                    else:
                        return CheckResult.wrong(
        ...
        for index, order in enumerate(orders):
            ...
            try:
                ...
                if 'Thanks for your order!' not in h2_elements:
                    return CheckResult.wrong(
                        'Order page should include <h2> tag with text "Thank you for your order!"'
                    )
